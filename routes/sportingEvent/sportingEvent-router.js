...
sportingEventRouter.post(
  ...
  (req, res, next) => {
    ...
    const message = !sportingEventName
      ? 'expected a sportingEventName'
      : !desc
        ? 'expected a desc'
        : null;

    if (message) return next(createError(400, `BAD REQUEST ERROR: ${message}`));

    new SportingEvent(req.body)
      .save()
...
sportingEventRouter.get('/api/sportingevents', async (req, res, next) => {
  ...
  SportingEvent.find()
    .then(sportingEvents => {
      if (!sportingEvents)
        return next(
          createError(404, 'NOT FOUND ERROR: sportingEvents not found')
        );
